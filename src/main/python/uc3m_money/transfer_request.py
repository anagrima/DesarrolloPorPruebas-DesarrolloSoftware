...
class TransferRequest:
    ...
    def guardar_json(self, ruta_archivo = "../../JsonFiles", utf=None):
        """Guarda la transferencia en un archivo JSON"""
        os.makedirs(ruta_archivo, exist_ok=True)
        ruta_fichero = os.path.join(ruta_archivo, "transferencias.json")

        try:
            with open(ruta_fichero, "r") as fichero:
                transferencias = json.load(fichero)
        except (FileNotFoundError, json.JSONDecodeError):
            transferencias = []

        transferencias.append(self.to_json())
        with open(ruta_fichero, "w", encoding=utf-8) as fichero:
            json.dump(transferencias, fichero, indent=4)
